...
# your real cluster url
cluster_url = "willluu.fmwmgpy.mongodb.net"

# FINAL WORKING URI
uri = f"mongodb+srv://{db_user}:{db_pass}@{cluster_url}/{db_name}?retryWrites=true&w=majority"
...
def insertData():
    ...
    try:
        ...
        print("Error:", e)
        return {"inserted": "failed", "error": str(e)}
...
def three():
    ...
    return {"results": list(results), "positive": positive_moods}

@app.route("/four")
def four():
    results = mongo.db.dataStuff.find().sort("event_name", 1)
    return {"results": list(results), "events": event_names}
...
def six():
    ...
    return {"results": list(results), "negative": negative_moods}
